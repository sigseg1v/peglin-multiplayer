# Multipeglin Uninstaller
# Cleanly removes BepInEx and Multipeglin from a Peglin installation.
#
# Usage:
...
if ($Help) {
    Write-Host "Multipeglin Uninstaller"
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  ./uninstall.ps1 /path/to/Peglin              Remove BepInEx + all mods"
    Write-Host "  ./uninstall.ps1 -ModsOnly /path/to/Peglin    Remove only Multipeglin"
    exit 0
}
...
if ($ModsOnly) {
    Write-Step "Removing Multipeglin only (keeping BepInEx)..."
    $ModDir = Join-Path $GameDir "BepInEx/plugins/Multipeglin"
    if (Test-Path $ModDir) { Remove-Item $ModDir -Recurse -Force }
    Write-Info "Removed BepInEx/plugins/Multipeglin/"
} else {
